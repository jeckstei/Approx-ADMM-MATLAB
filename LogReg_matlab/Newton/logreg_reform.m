function [ x, p, z, conv_log ] = logreg_reform( A, b, c, epsilon )
%   LOGREG_REFORM solve the logistic regression with l1 regularization
%   problem:
...
%         residual, inner iteration number at each outer iteration.
    ...
    while outer_iter < max_out_iter
        ...
        while inner_iter < max_inner_iter
            ...
            gradfw = -A'*assist;
            % gradf = [gradfv;gradfw]
            [~,x,g] = newton_reform([gradfv;gradfw],x,z,p,c,backtrack_coeff,backtrack_factor,ncols,A,b,aux);
        ...
        % logging the results
        conv_log.obj_val(outer_iter) = funeval_logreg(nu,aux,z);
        conv_log.prim_res(outer_iter) = norm(x(2:end)-z);
