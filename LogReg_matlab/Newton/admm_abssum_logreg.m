function [ x, p, conv_log ] = admm_abssum_logreg( A, b, c, ita, epsilon )
...
%         ita: a large positive number to relax the error torlerance at
%         begining phase.
%   Output:
...
%         residual, inner iteration number at each outer iteration.
    ...
    total_inner_iter = 0;
    outer_iter = 0;
    % these constants are used for absolute summable error conditions
    beta = 10^4 * sqrt(nrows);
    ...
    while outer_iter < max_out_iter
        ...
        while inner_iter < max_inner_iter
            ...
            % absolute summable type of error conditions
            if norm(g) < beta * (ita / (outer_iter+1)^2) / max(beta, norm(x)) || norm(g) < epsilon/10
