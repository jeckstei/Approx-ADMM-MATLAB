...
if ~QUIET
    fprintf('%3s\t%10s\t%10s\t%10s\t%10s\t%10s\n', 'iter', ...
      'r norm', 'gradf_norm', 's norm', 'eps_dual', 'objective');
end

for k = 1:MAX_ITER

    % x-update
    [x,gradf] = update_x(A, b, u, z, rho);
    ...
    %history.tol_grad(k) = opttol_logreg(gradf, m*mu,x,1e-6);
    history.gradf_norm(k) = norm(gradf);
    if ~QUIET
        fprintf('%3d\t%10.4f\t%10.4f\t%10.4f\t%10.4f\t%10.2f\n', k, ...
            history.r_norm(k), history.gradf_norm(k), ...
...
function [x, gradf] = update_x(A, b, u, z, rho, x0)
    % solve the x update
