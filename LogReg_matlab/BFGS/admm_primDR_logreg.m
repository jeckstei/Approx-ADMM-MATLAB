function [ x, p, conv_log ] = admm_primDR_logreg( A, b, c, sigma, epsilon, m )
...
%   A_data is the original data matrix.A_norm
%   Input parameters: 
...
%         residual, inner iteration number at each outer iteration.
    ...
    while outer_iter < max_out_iter
        ...
        while inner_iter < max_inner_iter
            ...
            xOld = x;
            gradOld = grad;
            [~,x,fVal,gradf,grad,objVal] = linesearch_p(x,z,p,fVal,grad,d,wolfeCondC1,wolfeCondC2,A,b,c,nu);
            % select p in  \partial f
            p = -gradf;
            % compute z
            z = x + (1/c) * p;
            z(2:end) = shrinkage(z(2:end), nu/c);
            inner_iter = inner_iter + 1;
            % termination condition for inner L-BFGS method.
            if norm( p -pOld - c*x + c*zOld) <= sigma * norm(-c*z + p +c * zOld - pOld) && inner_iter >1
                % update multiplier
                p = pOld + c * (x - zOld);
                break;
            end  
            if norm( gradf + c* (x-z+(1/c)*p) )<= epsilon && inner_iter >1
                % update multiplier
                p = pOld + c * (x - zOld);
                break;
            end             
        ...
        conv_log.dual_res(outer_iter) = norm(z - zOld);
