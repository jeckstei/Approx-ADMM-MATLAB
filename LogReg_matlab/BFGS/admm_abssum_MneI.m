function [ x, p, conv_log ] = admm_abssum_MneI( A, b, c, ita, K, power, epsilon, m )
...
%         residual, inner iteration number at each outer iteration.
    ...
    while outer_iter < max_out_iter
        ...
        while inner_iter < max_inner_iter
			...
			normg = norm(grad);
            if outer_iter <= K
                if normg <  ita / (outer_iter+1)  || (normg < epsilon / 10 && inner_iter > 1 )
                    break;
                end
            else
                if normg <  ita / (outer_iter+1)^power  || (normg < epsilon / 10 && inner_iter > 1 )
                    break;
                end                
            end
        end
