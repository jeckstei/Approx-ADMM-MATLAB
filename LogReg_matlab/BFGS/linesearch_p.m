function [ stepSize, xNext, funcNext, gradfNext,gradNext,objValNext] = linesearch_p( xStart, z, p, funcStart, gradStart, searchDirection, wolfeC1, wolfeC2, A, b, c, nu)
...
% step size should be nonnegative
    ...
    funcNext = funcStart;
    gradNext = gradStart;
    gradfNext = gradStart - c*(xStart-z+(1/c)*p);
...
%         end
        ...
        [objValTrial,funcTrial, gradfTrial,gradTrial] = funcval_logreg(xTrial,z,p,A,b,c,nu);
        if isnan(funcTrial)
            stepSize = 0;
            %xNext = xTrial;
            xNext = xStart;
