function [ x, p, conv_log ] = admm_relerr_MneI( A, b, c, sigma, epsilon, m )
%   ADMM_RELERR_LOGREG solve the logistic regression with l1 regularization
%   problem:
...
%         residual, inner iteration number at each outer iteration.
    ...
    while outer_iter < max_out_iter
        ...
        [~,fVal,~,grad] = funcval_MneI(x,z,p,A,b,c,nu);
        ...
        while inner_iter < max_inner_iter
            ...
            xOld = x;
            gradOld = grad;
            [~,x,fVal,gradf,grad,objVal] = linesearch_MneI(x,z,p,fVal,grad,d,wolfeCondC1,wolfeCondC2,A,b,c,nu);
            ...
            if ((2/c) * abs(dot((w-x),grad)) + sum(grad.^2) <= sigma * sum((x(2:end)-z).^2) || norm(grad) < epsilon/10) && inner_iter >1
                break;
            end            
        end
